...
Profile::Profile( uint64_t id_, uint64_t timestamp_,  std::vector<Contour>&& contours_,  std::vector<int>&& hierarchy_):
  id(id_)
, timestamp(timestamp_)
, contours(std::move(contours_))
, hierarchy(std::move(hierarchy_))
{}
...
void Profile::Set(uint64_t id_, uint64_t timestamp_,  std::vector<Contour>&& contours_, std::vector<int>&& hierarchy_)
{
  id = id_;
  timestamp = timestamp_;
  contours.clear();
