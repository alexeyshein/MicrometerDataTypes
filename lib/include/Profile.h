...
public:
    ...
    Profile( uint64_t id, uint64_t timestamp,  std::vector<Contour>&& contours,  std::vector<int>&& hierarchy);
	~Profile() = default;
    void Set(uint64_t id, uint64_t timestamp,  const std::vector<Contour>& contours, const std::vector<int>& hierarchy);
    void Set(uint64_t id, uint64_t timestamp,  std::vector<Contour>&& contours, std::vector<int>&& hierarchy);
    ...
    void SetContours(std::vector<Contour>&& contours_){contours = std::move(contours_);}
