...
public:
    ...
    Contour( uint64_t id, std::deque<Point2d<double>>&& points,  ContourType type);
    ...
    void Set(uint64_t id,  std::deque<Point2d<double>>&& points,  ContourType type);
    ...
    void SetPoints(std::deque<Point2d<double>>&& points_){points = std::move(points_);}
